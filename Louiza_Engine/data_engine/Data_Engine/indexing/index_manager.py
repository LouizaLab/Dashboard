...
class IndexManager:
    ...
    def index_batch(self, records: List[DataRecord], embeddings: Optional[np.ndarray] = None) -> bool:
        ...
        try:
            ...
            # Store all records (batch save at the end)
            # Check if storage_backend has batch_save method
            if hasattr(self.storage_backend, 'save_records_batch'):
                self.storage_backend.save_records_batch(records)
            else:
                # Fallback: save individually but defer index saves
                for record in records:
                    self.storage_backend.save_record(record, defer_save=True)
                # Save index once at the end
                if hasattr(self.storage_backend, '_save_index'):
                    self.storage_backend._save_index()
            
            # Index metadata for all records
            for record in records:
                self.metadata_store.index_record(record)
            # Save metadata index once at the end
            self.metadata_store._save_index()
