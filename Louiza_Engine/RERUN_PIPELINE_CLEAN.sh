#!/bin/bash
# Complete pipeline rerun with cleaned/normalized brands
# This script: Recompiles data → Personas → Simulation → Anchoring → Visualizations

set -e  # Exit on error

DATA_VERSION="data_2026_01_09_run01_clean"
PERSONA_VERSION="PersonaSet_v1_clean.json"
BASELINE_RUN_DIR="runs/baseline_52w_clean"
ANCHORED_RUN_DIR="runs/anchored_52w_clean"
PLOTS_DIR="plots/baseline_52w_clean"

echo "=========================================="
echo "Louiza Engine - Clean Pipeline Rerun"
echo "=========================================="
echo "Data Version: $DATA_VERSION"
echo ""

# Step 1: Recompile Data with Brand Normalization
echo "Step 1: Recompiling data with brand normalization..."
...
if [ ! -d "data/synthetic/$DATA_VERSION" ]; then
...
# Check brand quality
echo ""
echo "Checking brand quality..."
python3 << EOF
import pandas as pd
brands = pd.read_csv('data/synthetic/$DATA_VERSION/brands.csv')
print("\nBrands after normalization:")
print(brands.to_string(index=False))
print(f"\nTotal brands: {len(brands)}")
EOF

echo ""
echo "✓ Data recompiled: $DATA_VERSION"
echo ""
