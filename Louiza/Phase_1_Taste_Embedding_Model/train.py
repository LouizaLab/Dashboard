...
import os
# Set environment variables to avoid tokenizer parallelism and threading issues on macOS
os.environ['TOKENIZERS_PARALLELISM'] = 'false'
os.environ['OMP_NUM_THREADS'] = '1'
os.environ['MKL_NUM_THREADS'] = '1'
os.environ['NUMEXPR_NUM_THREADS'] = '1'

# Import torch first to set threading before other imports
import torch
...
# Import sentence-transformers with explicit settings
import warnings
...
def main():
    ...
    # Initialize text encoder with explicit settings to avoid macOS mutex issues
    ...
    # Use a lock to ensure single-threaded initialization
    init_lock = threading.Lock()
    
    def load_encoder():
        with init_lock:
            try:
                # Try loading with explicit device and single-threaded settings
                ...
                try:
                    ...
                    print(f"  Error in fallback attempt: {e2}")
                    raise
    
    try:
        text_encoder = load_encoder()
        text_dim = text_encoder.get_sentence_embedding_dimension()
        print(f"✓ Text encoder loaded successfully (dimension: {text_dim})")
    except Exception as e:
        ...
        print("  3. On macOS, you may need to set: export TOKENIZERS_PARALLELISM=false")
        raise
