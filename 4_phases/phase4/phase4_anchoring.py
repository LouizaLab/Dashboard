...
class ModelFineTuner:
    ...
    def _load_models(self):
        ...
        if os.path.exists(self.phase1_model_path):
            try:
                checkpoint = torch.load(self.phase1_model_path, map_location=self.device, weights_only=False)
        ...
        if os.path.exists(self.phase2_model_path):
            try:
                checkpoint = torch.load(self.phase2_model_path, map_location=self.device, weights_only=False)
