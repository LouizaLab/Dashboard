    ...
    <div className="w-64 bg-dark-surface border-r border-dark-border p-4 overflow-y-auto">
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between pt-4 border-t border-dark-border">
          <label className="text-sm font-medium text-gray-300">Use GPT</label>
          <button
            onClick={() => updateFilter('use_gpt', !filters.use_gpt)}
            className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
              filters.use_gpt ? 'bg-accent-primary' : 'bg-gray-600'
            }`}
          >
            <span
              className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                filters.use_gpt ? 'translate-x-6' : 'translate-x-1'
              }`}
            />
          </button>
        </div>
