...
function ResultsPanel({ results, onAgentSelect }) {
  ...
  const loadAgents = async () => {
    try {
      const response = await fetch('http://localhost:8000/api/agents/?limit=20');
          ...
          <div className="grid grid-cols-2 gap-3">
            {agents.slice(0, 8).map((agent) => (
              <button
                ...
                className="bg-dark-hover border border-dark-border rounded-lg p-3 text-left hover:border-accent-primary hover:bg-dark-surface transition-all"
              >
                <div className="font-medium text-gray-200 text-sm">{agent.display_name}</div>
                <div className="text-xs text-gray-400 mt-1">{agent.archetype_display}</div>
              </button>
            ))}
          </div>
        </div>
