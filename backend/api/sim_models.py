...
class PersonaAgent(models.Model):
    ...
    ARCHETYPE_CHOICES = [
        ...
        ("convenience_loyalist", "Convenience Loyalist"),
        ("late_night_craver", "Late-night Craver"),
        ("trend_chaser", "Trend Chaser"),
        ("family_bundle_buyer", "Family Bundle Buyer"),
        ("protein_maximizer", "Protein Maximizer"),
    ]
...
class EvidenceSurveyDatum(models.Model):
    ...
    def save(self, *args, **kwargs):
        # Auto-update dataset_name to match the date year if it's the old default
        if self.dataset_name == "Fast-Food Consumer Survey 2024" and self.date.year != 2024:
            self.dataset_name = f"Fast-Food Consumer Survey {self.date.year}"
