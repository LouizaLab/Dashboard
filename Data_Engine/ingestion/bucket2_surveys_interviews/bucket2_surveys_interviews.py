...
class SurveysInterviewsIngester(IngestionBase):
    ...
    def _chunk_text(self, text: str, chunk_size: int, chunk_overlap: int) -> list:
        """
        Split text into chunks, prioritizing User response boundaries.
        
        For interview transcripts, chunks at User response boundaries to avoid
        mixing Agent prompts with User responses.
        """
        import re
        
        # Try conversation-aware chunking for interview transcripts
        # Look for "User:" patterns to identify response boundaries
        user_pattern = re.compile(r'User:\s*', re.IGNORECASE)
        user_matches = list(user_pattern.finditer(text))
        
        if len(user_matches) > 1:  # If we have multiple User responses, chunk by them
            chunks = []
            for i, match in enumerate(user_matches):
                # Start of this User response
                start = match.start()
        ...
        # Fallback to fixed-size chunking if no User patterns found
        chunks = []
        start = 0
